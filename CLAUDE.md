...
Result stability was removed at the user's request on 2026-09-05. Do not
reintroduce its panel, warnings, or payload. Executed membership remains a disclosure.
...
**Result stability was removed on 2026-09-05 at the user's explicit request.** The panel, warnings, measurement payloads, and orphan result-removal diagnostic helpers are gone. Historical evidence remains in `docs/design-notes.md`. Cohort pooling, exclusions, per-member embedding cosines, independent A/B comparison, and its Jaccard overlap remain. The pooled scorer preserves its matrix-product accumulation path to avoid rounding-induced changes to near-tied ranks; see `docs/interface-simplification.md`.

`LOW_N_THRESHOLD` is 5 and survives, because that question - how large should a cohort be - is the one the curve can answer honestly, and the picker is the one place where size is all that is known. `precompute/validate_cohorts.py` check 5 still measures the curve and now **fails** if the knee moves away from 5, but only on a full sweep: on a `--cohorts 40` sample the knee lands at 10, from buckets standing on one cohort each, which is exactly the noise the bucketing exists to survive. `cohorts.STABILITY_FLOOR` remains 0.70 for that offline validation.
...
**A comparison runs two independent pooled queries.** Inline summaries retain both actual included counts and A/B role dots. Selectors name the cohorts; the network and map preserve their names and marks. Sample exclusions belong to cohort A's editable selection. The selected and executed membership remain separate because the selection can change after a search. The right inspector opens both cohort definitions after comparison searches. When a match or study is selected, both cohort names and counts remain as expandable summaries above its metadata.
