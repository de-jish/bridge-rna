...
### Finding a sample among 942,563

`manifold/find.py` resolves an identifier to the points it names: a GEO sample (`GSM…`), a GEO series (`GSE…`), an OSDR study (`OSD-###`), or an OSDR sample by its full `<study>|<name>` key or its bare name.
