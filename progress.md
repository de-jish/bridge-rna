...
Validation: 508 pytest tests and 487 browser checks pass on the merged tree (205 map/sample, 178 cohort, 70 upload, 31 network style, 3 cleanup), with no console errors in any suite.

Two browser suites were failing before this merge and were fixed, neither caused by it. `e2e_network_style.py` selected a Color by option by counting arrow presses from whatever held focus; Dash 4 renders each option as its own checkbox input, so an arrow key moves DOM focus without moving a selection cursor, and the screenshot taken just above re-settled focus inside the open menu - it re-selected the value already set and timed out on every run. It now focuses the named option and presses Enter, which is the same keyboard assertion and is deterministic. `e2e_cleanup_check.py` asserted the neighborhood overview contains "GEO studies", one of the three statistic cards removed on 2026-09-08, and still defaulted to port 8050. Both take a `--url`/`--port` and start no server of their own, which is why they went unrun while the two self-starting suites stayed green. Contrast recomputed from the stylesheet: text-muted 7.09:1 on a panel, text-primary 16.54:1 on the canvas, accent-text 10.04:1, white on `--action-primary` 4.61:1. NASA brand red `#f64137` is 3.67:1 and is never used to carry a white label.
...
A permanent OSD-137 comparison path switches from `Liver · Basal Control` to `Liver · Ground Control`: both arms have their own distinct 250-row evidence result (157 shared), the requested-hit traces remain unchanged, and the search callback still runs only once.
