...
- Give each action one clear entry point. Do not expose graph selection tools without a consumer for the selection. Prefer inline counts over standalone statistic cards; when a diagnostic feature is explicitly retired, remove its UI, callback, payload, and orphan styles while preserving independently used scientific validation.
